...
with supplier_data:
    ...
    with right:
        st.write(pd.DataFrame(supplier.drop(columns = "id")[((supplier["Score Ranked by the IHG"]>=ihg) | (supplier["Score Ranked by the IHG"]!=supplier["Score Ranked by the IHG"])) & (supplier["Country"]==country)].index.to_list()))
    st.markdown("\n")
